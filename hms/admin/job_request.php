...
<body>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            // Fetch and display job requests
            show();
            function show() {
                $.ajax({
            ...
            $(document).on('click', '.approve', function () {
                const id = $(this).attr('id'); // Get the ID of the clicked button
                $.ajax({
                    url: "ajax_approve.php", // Endpoint for approving
                    method: "POST",
                    data: { id: id },
                    success: function (response) {
                        alert("Approved successfully!");
                        show(); // Reload the table after action
                    },
                    error: function () {
            ...
            $(document).on('click', '.reject', function () {
                const id = $(this).attr('id'); // Get the ID of the clicked button
                $.ajax({
                    url: "ajax_reject.php", // Endpoint for rejecting
                    method: "POST",
                    data: { id: id },
                    success: function (response) {
